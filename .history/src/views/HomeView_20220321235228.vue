...
export default {
  ...
  methods: {
    getapi() {
      ...
      this.axios.get('http://localhost:1016/posts/page/list', { pageNum: 1, pageSize: 'sss' }).then((res) => {
        const rs = res.data;
    ...
    loadoption() {
      ...
      // 详细：paginate 可以调用自定义事件。可以在组件上收听并运行您的回调。他会回传一个参数page，为当前页码。可以利用这个参数来进行列表内容的更新。
      const params = { pageNum: this.pageNo, pageSize: this.perPage };
      this.axios.get('http://localhost:1016/posts/page/list', { pageNum: 1, pageSize: 5 }).then((res) => {
        // 在此更新数据
        const rs = res.data;
        this.post = rs.data.data;
        this.total = rs.data.total;
